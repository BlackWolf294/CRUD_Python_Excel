...
Rut=r".\Datos_Crud.xlsx"


def leer(ruta:str, extraer:str):
    Archivo_Excel = load_workbook(ruta)
    Hoja_datos = Archivo_Excel['Datos_Del_Crud_Productos']
        ...
        if i[0].value==identificador:
            ...
            for d in datos_actualizados:
                if d=='titulo' and not (datos_actualizados[d]==''):
                    ...
                    hoja.cell(row=fila, column=Precio_unidad).value=datos_actualizados[d]
                elif  d=='Precio_unidad' and not(datos_actualizados[d]==''):
...
def agregar(ruta:int,datos:dict):
    ...
    for i in Hoja_datos:
        ...
        if not(isinstance(i[0].value,int)):
            ...
            hoja.cell(row=identificador, column=Precio_unidad).value=datos['Precio_unidad']
...
def borrar(ruta,identificador):
    ...
    for i in Hoja_datos:
        if i[0].value==identificador:
            ...
            hoja.cell(row=fila, column=Precio_unidad).value=""
...
datosActualizados={'producto':'','categoria':'','Precio_unidad':'','cantidad':''}
while True:
    ...
    if not(accion=='1') and not (accion=='2') and not (accion=='3') and not (accion=='4'):
        ...
        print()
        print('* Precio *')
        datosActualizados['Precio_unidad']=input('indique el precio del producto : ')
